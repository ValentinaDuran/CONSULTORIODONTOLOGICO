...
namespace CONSULTORIODONTOLOGICO.Server.Controllers
{
    ...
    public class ObrasSocialesController : ControllerBase
    {
        ...
        [HttpPost]
       /*public async Task<ActionResult<ObraSocial>> Post (ObraSocial obrasocial )
        {
            try
            {
        ...
        public ActionResult Put(int id, [FromBody] ObraSocial obrasocial)
        {
            ...
            amnis.NombreObraSocial = obrasocial.NombreObraSocial;

            try
            {
                context.ObrasSociales.Update(amnis);
                context.SaveChanges();
                return Ok();
            }
            catch (Exception o)
